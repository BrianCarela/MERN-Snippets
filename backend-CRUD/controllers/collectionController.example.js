...
async function getOnecollectionName (req, res) {
    try {
        let result = await collectionName.findOne({propertyName: req.params.propertyName});
...
async function updateOnecollectionName(req, res){
    try {
        let targetcollectionName = await collectionName.findOne({ propertyName: req.params.propertyName })

        // ternaries avoid inputting undefined values
        let updatedcollectionName = {
            propertyName: req.body.propertyName ? req.body.propertyName : targetcollectionName.propertyName,
        }
